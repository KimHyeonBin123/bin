...
if uploaded is not None:
    ...
    st.error("CSV를 찾을 수 없습니다. 업로드 해주세요.")
    st.stop()
...
def item_icon(name: str):
    return f"https://ddragon.leagueoflegends.com/cdn/13.20.1/img/item/{name}.png"
...
for i in range(0, len(g), cols_per_row):
    ...
    for j, (_, r) in enumerate(row.iterrows()):
        cols[j].image(item_icon(r['item']), width=32, use_column_width=False)
...
if "spell_combo" in dfc.columns and dfc["spell_combo"].str.strip().any():
    ...
    def spell_icon(name: str):
        return f"https://ddragon.leagueoflegends.com/cdn/13.20.1/img/spell/{name}.png"
